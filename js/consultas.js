    ...
    querySnapshot.forEach(function (doc) {
        ...
        ANIMALFINAL.map((animal) => {
            const {
                ...
                //lenar select de colores
            $("#selectcolores").append(`
                <option value="${colorImagen}" style="color:${colorImagen}">${colorImagen}</option>
            `);


            //al cambiar de colorImagen

            $("#selectcolores").change((e) => {
                e.preventDefault();
               
                
               colorImagen===e.target.value &&  $("#cardlistanimales").html(`<div style="border: 8px solid ${colorImagen}" class="card cardanimales">
